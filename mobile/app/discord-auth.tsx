...
type Step = 'idle' | 'validating' | 'liveness_loading' | 'liveness_webview' | 'finalizing' | 'done' | 'error';
...
export default function DiscordAuthScreen() {
  ...
  async function beginAuth() {
    ...
    if (normalizedCode.length !== 4) {
      setError('Enter the 4-character code from Discord.');
      return;
    }
    ...
    setStep('validating');
    try {
      const info = await api.discordStartAuth(normalizedCode);
      setParticipantLabel(
        info.displayName || (info.discordUsername ? `@${info.discordUsername}` : null),
      );
      setStep('liveness_loading');
      const { livenessSessionId: id } = await api.discordLivenessStart();
      setLivenessSessionId(id);
      setCode(normalizedCode);
      setStep('liveness_webview');
    } catch (err: any) {
      setError(err.message || 'Could not start Discord authentication');
      setStep('error');
    }
  }
  ...
  async function handleLivenessComplete() {
    ...
    setStep('finalizing');
    try {
      const result = await api.discordCompleteAuth(code, livenessSessionId);
    ...
    <SafeAreaView className="flex-1 bg-bg">
      <View className="flex-1 px-6 pt-8 pb-10 justify-between">
        <View className="gap-5">
          <Text className="text-ink text-3xl font-bold">Discord Auth</Text>
          <Text className="text-muted text-base">
            Generate a 4-character code in Discord, then paste it here to verify for that channel session.
          </Text>

          {(step === 'idle' || step === 'validating' || step === 'error') && (
            <View>
              <Text className="text-ink text-sm font-medium mb-2">Discord Code</Text>
              <TextInput
                value={code}
                onChangeText={(value) => setCode(value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 4))}
          ...
          {participantLabel && step !== 'idle' && (
            <Text className="text-muted text-sm">Authenticating for {participantLabel}</Text>
          )}

          {(step === 'validating' || step === 'liveness_loading' || step === 'finalizing') && (
            <View className="items-center mt-6">
              ...
              <Text className="text-muted text-base mt-3">
                ...
                {step === 'finalizing' && 'Finalizing Discord verification...'}
              </Text>
            </View>
          )}
