...
export default function Home() {
  const [user, setUser] = useState<{ legalName: string; email: string } | null>(
    null,
  );
    ...
    <SafeAreaView className="flex-1 bg-bg">
      <View className="flex-1 px-6 pt-6">
        ...
        <PrimaryButton
          label="Telegram Auth"
          variant="outline"
          onPress={() => router.push("/telegram-auth")}
        />
        ...
        <PrimaryButton
          ...
          onPress={async () => {
            try {
              const { sessionId, peerName, peerPhoto } =
                await api.testStartSession();
              router.push({
                pathname: "/verify",
                params: {
                  sessionId,
        ...
        <Text className="text-ink text-lg font-semibold mt-10 mb-4">
          Recent
        </Text>
