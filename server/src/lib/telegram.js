...
export const sendMessage = (chatId, text, options = {}) =>
  apiCall('sendMessage', { chat_id: chatId, text, parse_mode: 'HTML', ...options });

export const editMessage = (chatId, messageId, text, options = {}) =>
  apiCall('editMessageText', { chat_id: chatId, message_id: messageId, text, parse_mode: 'HTML', ...options });

export const answerCallbackQuery = (callbackQueryId, text, showAlert = false) =>
  apiCall('answerCallbackQuery', { callback_query_id: callbackQueryId, text, show_alert: showAlert });
...
export async function createSession(chatId, hostUserId, meetCode = null) {
...
export function renderStatusMessage(session, participants) {
  const title = session.status === 'active' ? '🛡 <b>NAI Verification Session</b>' : '🏁 <b>Session Ended</b>';
...
export function generateAuthDeepLink(tgUserId, tgSessionId) {
  ...
  const baseUrl = process.env.NAI_PUBLIC_AUTH_BASE_URL || process.env.CLIENT_URL;
...
export async function isGroupAdmin(chatId, userId) {
  ...
  try {
    const member = await apiCall('getChatMember', { chat_id: chatId, user_id: userId });
