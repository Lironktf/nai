...
router.post("/interactions", (req, res) => {
  ...
  if (
    !signature ||
    !timestamp ||
    !verifyDiscordSignature(rawBody, String(signature), String(timestamp))
  ) {
  ...
  handleInteraction(interaction, res).catch((err) => {
    ...
    jsonResponse(
      res,
      interactionMessage("Discord bot error.", { ephemeral: true }),
    );
  });
});
...
async function handleInteraction(interaction, res) {
  ...
  return jsonResponse(
    res,
    interactionMessage("Unsupported Discord interaction.", { ephemeral: true }),
  );
}
...
async function handleApplicationCommand(interaction, res) {
  ...
  if (command === "nai_start") {
    ...
    if (session) {
      return jsonResponse(
        res,
        interactionMessage("A session is already active in this channel.", {
          ephemeral: true,
        }),
      );
    }
    ...
    if (
      ...
      (!Number.isInteger(minutes) ||
        minutes < MIN_REAUTH_MINUTES ||
        minutes > MAX_REAUTH_MINUTES)
    ) {
      return jsonResponse(
        res,
        interactionMessage(
    ...
    return jsonResponse(
      res,
      interactionMessage("NAI verification session started.", {
        ephemeral: true,
      }),
    );
  }
  ...
  if (command === "nai_status") {
    ...
    if (!session) {
      return jsonResponse(
        res,
        interactionMessage("No active session found in this channel.", {
          ephemeral: true,
        }),
      );
    }
    ...
    return jsonResponse(
      res,
      interactionMessage("Session status refreshed.", { ephemeral: true }),
    );
  }
  ...
  if (command === "nai_end") {
    ...
    return jsonResponse(
      res,
      interactionMessage("NAI verification session ended.", {
        ephemeral: true,
      }),
    );
  }
  ...
  return jsonResponse(
    res,
    interactionMessage(`Unknown command: ${command}`, { ephemeral: true }),
  );
}
...
async function handleComponentInteraction(interaction, res) {
  ...
  if (action === "auth") {
    const participant = await issueParticipantAuthCode(
      sessionId,
      discordUser,
      member,
    );
  ...
  if (action === "refresh") {
    ...
    return jsonResponse(
      res,
      interactionMessage("Status refreshed.", { ephemeral: true }),
    );
  }
  ...
  if (action === "reverify_me") {
    ...
    return jsonResponse(
      res,
      interactionMessage("You have been marked for re-verification.", {
        ephemeral: true,
      }),
    );
  }
  ...
  return jsonResponse(
    res,
    interactionMessage("Unknown action.", { ephemeral: true }),
  );
}
...
router.get("/sessions/current", requireAuth, async (req, res) => {
  ...
  if (linksError) {
    console.error("[discord/sessions/current] link lookup error:", linksError);
    return res.status(500).json({ error: "Failed to load Discord sessions" });
  }

  const discordUserIds = (accountLinks ?? []).map((row) => row.discord_user_id);
  if (!discordUserIds.length) return res.json([]);

  const { data: participants, error: participantsError } = await supabase
    .from("discord_session_participants")
    .select(
      "id, discord_session_id, discord_user_id, discord_username, display_name, status, last_verified_at, verification_expires_at, updated_at",
    )
  ...
  if (participantsError) {
    console.error(
      "[discord/sessions/current] participant lookup error:",
      participantsError,
    );
    return res.status(500).json({ error: "Failed to load Discord sessions" });
  }
  ...
  const sessionIds = [
    ...new Set(
      (participants ?? []).map((row) => row.discord_session_id).filter(Boolean),
    ),
  ];
  ...
  const { data: sessions, error: sessionsError } = await supabase
    .from("discord_verification_sessions")
    .select(
      "id, discord_channel_id, discord_guild_id, meet_code, status, reauth_interval_minutes, started_at, ended_at",
    )
    .in("id", sessionIds)
    .eq("status", "active");
  ...
  if (sessionsError) {
    console.error(
      "[discord/sessions/current] session lookup error:",
      sessionsError,
    );
    return res.status(500).json({ error: "Failed to load Discord sessions" });
  }
  ...
  const items = (participants ?? [])
    .map((participant) => {
      const session = sessionById.get(participant.discord_session_id);
...
router.post("/mobile/start-auth", requireAuth, async (req, res) => {
  ...
  if (
    ...
    return res
      .status(410)
      .json({ error: "Code expired. Generate a new code from Discord." });
  }
...
router.post("/mobile/complete-auth", requireAuth, async (req, res) => {
  ...
  const expiresAtIso = new Date(
    Date.now() + getDiscordReauthMinutes(session) * 60 * 1000,
  ).toISOString();
  ...
  if (partErr) {
    ...
    return res
      .status(500)
      .json({ error: "Failed to update Discord participant status" });
  }
