...
// GET /mobile/public-config
// Exposes only client-safe configuration needed by browser/mobile webviews.
...
router.get("/me", requireAuth, async (req, res) => {
  ...
  if (user.profile_photo_s3_key) {
    photoUrl = await getPresignedUrl(user.profile_photo_s3_key, 300).catch(
      () => null,
    );
  }
...
router.post("/kyc/start", requireAuth, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(502)
      .json({ error: "Failed to initiate identity verification" });
  }
});
...
router.post("/passkey/register/complete", requireAuth, async (req, res) => {
  ...
  try {
    registrationInfo = await verifyEnrollmentChallenge(
      userId,
...
router.post("/passkey/assert/start", requireAuth, async (req, res) => {
  ...
  if (session.state !== "awaiting_both") {
    return res.status(409).json({
      error: "Session is not ready for authentication",
      state: session.state,
    });
  }
...
router.post("/passkey/assert/complete", requireAuth, async (req, res) => {
  ...
  if (!sessionId || !assertionResponse) {
    return res
      .status(400)
      .json({ error: "Missing sessionId or assertionResponse" });
  }
  ...
  if (session.state !== "awaiting_both") {
    return res
      .status(409)
      .json({ error: "Session is not in awaiting_both state" });
  }
  ...
  const bothDone =
    updated?.requester_verified_at && updated?.recipient_verified_at;
...
router.post("/face/enroll", requireAuth, async (req, res) => {
  ...
  if (user.status !== "pending_video") {
    return res.status(409).json({
      error: "Face enrollment not applicable for current status",
      status: user.status,
    });
  }
...
router.post("/face/activate-bypass", requireAuth, async (req, res) => {
  ...
  if (!user.profile_photo_s3_key) {
    return res.status(422).json({
      error:
        "No KYC reference photo found. Complete identity verification first.",
    });
  }

  try {
    const { passed, score } = await compareFaces(
      user.profile_photo_s3_key,
      imageBase64,
    );
    console.log(
      `[face/activate-bypass] user=${userId} score=${score.toFixed(1)} passed=${passed}`,
    );
    ...
    return res
      .status(422)
      .json({ error: err.message, passed: false, score: 0 });
  }
});
...
router.post("/verification/request", requireAuth, async (req, res) => {
  ...
  if (recipientId === userId)
    return res.status(400).json({ error: "Cannot verify with yourself" });
  ...
  const requesterPhotoUrl = requester?.profile_photo_s3_key
    ? await getPresignedUrl(requester.profile_photo_s3_key, 300).catch(
        () => null,
      )
    : null;
...
router.post("/verification/accept", requireAuth, async (req, res) => {
  ...
  if (!session) return res.status(404).json({ error: "Session not found" });
  if (session.recipient_id !== userId)
...
router.get("/verification/session/:id", requireAuth, async (req, res) => {
  ...
  const { data: session } = await supabase
    .from("verification_sessions")
    .select(
      "state, verification_code, requester_id, recipient_id, verified_at, fail_reason",
    )
    .eq("id", id)
    .single();
  ...
  const results = await Promise.all(
    (sessions ?? []).map(async (s) => {
      ...
      const peerPhotoUrl = peer?.profile_photo_s3_key
        ? await getPresignedUrl(peer.profile_photo_s3_key, 300).catch(
            () => null,
          )
        : null;
      return {
...
router.post("/liveness/complete", requireAuth, async (req, res) => {
  ...
  try {
    console.log(
      `[liveness/complete] user=${userId} session=${sessionId} starting`,
    );
    ...
    console.log(
      `[liveness/complete] user=${userId} session=${sessionId} result`,
      result,
    );
...
// POST /mobile/post-kyc/activate
// Completes an AWS liveness session for a newly KYC-approved user and promotes
// the account to active if liveness + face match pass.
router.post("/post-kyc/activate", requireAuth, async (req, res) => {
  const { userId } = req.user;
  ...
  if (!["pending_video", "pending_passkey"].includes(user.status)) {
    return res.status(409).json({
      error: "Post-KYC activation not applicable for current status",
      status: user.status,
    });
  }
  ...
  if (!user.profile_photo_s3_key) {
    return res.status(422).json({
      error:
        "No KYC reference face found. Please complete identity verification first.",
    });
  }
  ...
  try {
    ...
    if (!result.livenessPass || !result.faceMatchPassed) {
      return res.status(422).json({
        error: "Liveness or face match did not pass",
        ...result,
      });
    }
    ...
    await supabase.from("audit_logs").insert({
      user_id: userId,
      event_type: "POST_KYC_LIVENESS_ACTIVATED",
      metadata: {
        liveness_confidence: result.livenessConfidence,
    ...
    console.error("[post-kyc/activate] error:", err.message);
    return res.status(422).json({
      error: err.message,
      livenessPass: false,
      faceMatchPassed: false,
...
// POST /mobile/post-kyc/finalize
// Marks a post-KYC user active after the client has already completed and
// validated liveness + face match through /mobile/liveness/complete.
router.post("/post-kyc/finalize", requireAuth, async (req, res) => {
  const { userId } = req.user;
  ...
  if (!["pending_video", "pending_passkey"].includes(user.status)) {
    return res.status(409).json({
      error: "Post-KYC finalize not applicable for current status",
      status: user.status,
    });
  }

  await supabase.from("users").update({ status: "active" }).eq("id", userId);
  console.log(`[post-kyc/finalize] user=${userId} status=active`);
  await supabase.from("audit_logs").insert({
    user_id: userId,
    event_type: "POST_KYC_FINALIZED",
    ip_address: req.ip,
    user_agent: req.headers["user-agent"],
  });
...
router.post("/test/sync-profile-photo", requireAuth, async (req, res) => {
  ...
  if (!iv?.persona_inquiry_id) {
    return res
      .status(404)
      .json({ error: "No approved KYC inquiry found for this user" });
  }

  const selfieUrl = await fetchSelfiePhotoUrl(iv.persona_inquiry_id);
  if (!selfieUrl) {
    return res.status(422).json({
      error: `Persona returned no selfie URL for inquiry ${iv.persona_inquiry_id}`,
    });
  }
  ...
  if (!imgRes.ok) {
    return res.status(502).json({
      error: `Failed to download selfie from Persona (${imgRes.status})`,
    });
  }
  ...
  await supabase
    .from("users")
    .update({ profile_photo_s3_key: key })
    .eq("id", userId);
...
router.post("/passkey/register/bypass", requireAuth, async (req, res) => {
  ...
  if (!["pending_video", "pending_passkey"].includes(user?.status)) {
    return res.status(409).json({
      error: "Bypass not applicable for current status",
      status: user?.status,
    });
  }
...
router.post("/test/start-session", requireAuth, async (req, res) => {
  ...
  if (userId === ALICE_ID) {
    return res
      .status(400)
      .json({ error: "Log in as a non-Alice user to test." });
  }
...
router.post("/test/assert-bypass", requireAuth, async (req, res) => {
  ...
  if (updated?.requester_verified_at && updated?.recipient_verified_at) {
    ...
    console.log(
      `[test] assert-bypass finalised session ${sessionId} → ${code}`,
    );
  }
