...
const startSessionSchema = z.object({
  ...
  reauthIntervalMinutes: z.number().int().min(MIN_REAUTH_MINUTES).max(MAX_REAUTH_MINUTES).optional(),
});
...
async function ensureHostAccess(sessionId, userId) {
  const { data: session } = await supabase
    ...
    .select('id, host_user_id, status, meeting_code, reauth_interval_minutes, started_at, ended_at')
    .eq('id', sessionId)
    .single();

  if (!session) return { error: { status: 404, message: 'Session not found' } };
  if (session.host_user_id !== userId) return { error: { status: 403, message: 'Host access required' } };
  return { session };
}
...
async function ensureParticipantAccess(sessionId, userId) {
  ...
  if (!participant) {
    return { error: { status: 403, message: 'Join the meeting session first' } };
  }
...
async function recordMeetingEvent({ sessionId, participantId = null, eventType, metadata = {} }) {
...
function mapParticipantRow(row) {
  ...
  return {
    ...
    linkedFullName,
    linkedEmail,
    identityLabel: [row.display_name, linkedFullName ?? linkedEmail].filter(Boolean).join(' | ') || 'Unknown',
...
router.post('/session/start', requireAuth, async (req, res) => {
  ...
  if (existingActive) {
    return res.status(409).json({ error: 'An active session already exists for this meeting code', sessionId: existingActive.id });
  }
  ...
  const { data: session, error } = await supabase
    ...
    .select('id, meeting_code, host_user_id, status, reauth_interval_minutes, started_at, ended_at, created_at, updated_at')
    .single();
...
router.get('/session/:sessionId', requireAuth, async (req, res) => {
  ...
  if (hostAccess.error) return res.status(hostAccess.error.status).json({ error: hostAccess.error.message });
...
router.get('/session/:sessionId/participants', requireAuth, async (req, res) => {
  const { sessionId } = req.params;
  const hostAccess = await ensureHostAccess(sessionId, req.user.userId);
  if (hostAccess.error) return res.status(hostAccess.error.status).json({ error: hostAccess.error.message });
...
router.post('/session/:sessionId/verify-all', requireAuth, async (req, res) => {
  ...
  if (error) {
    ...
    return res.status(500).json({ error: 'Failed to mark participants for reverification' });
  }
  ...
  await emitMeetingEvent(sessionId, 'meeting:participants-updated', { reason: 'verify-all' });

  return res.json({ ok: true });
});

// POST /meet/session/:sessionId/participant/:participantId/reverify
router.post('/session/:sessionId/participant/:participantId/reverify', requireAuth, async (req, res) => {
  ...
  await recordMeetingEvent({
    ...
    metadata: { hostUserId: req.user.userId, reason: bodyParsed.data.reason ?? null },
  });
...
router.post('/join', requireAuth, async (req, res) => {
  ...
  if (!session) return res.status(404).json({ error: 'No active meeting session found for this code' });
  ...
  const { data: participant, error } = await supabase
    .from('meeting_participants')
    .upsert(participantPayload, { onConflict: 'meeting_session_id,nai_user_id' })
  ...
  return res.json({
    ...
    reauthIntervalMinutes: session.reauth_interval_minutes ?? DEFAULT_REAUTH_MINUTES,
  });
});

// POST /meet/session/:sessionId/liveness/start
router.post('/session/:sessionId/liveness/start', requireAuth, async (req, res) => {
  const { sessionId } = req.params;
  const participantAccess = await ensureParticipantAccess(sessionId, req.user.userId);
  if (participantAccess.error) {
    return res.status(participantAccess.error.status).json({ error: participantAccess.error.message });
  }
...
router.post('/session/:sessionId/liveness/complete', requireAuth, async (req, res) => {
  ...
  try {
    const result = await getLivenessResult(livenessSessionId, user.profile_photo_s3_key);
...
router.post('/session/:sessionId/passkey/assert/start', requireAuth, async (req, res) => {
  const { sessionId } = req.params;
  ...
  const challengeOptions = await generateAuthChallenge(req.user.userId, creds);
  return res.json({ challengeOptions });
});

// POST /meet/session/:sessionId/passkey/assert/complete
router.post('/session/:sessionId/passkey/assert/complete', requireAuth, async (req, res) => {
...
router.post('/session/:sessionId/complete-auth', requireAuth, async (req, res) => {
  ...
  const reauthIntervalMinutes = session.reauth_interval_minutes ?? DEFAULT_REAUTH_MINUTES;
