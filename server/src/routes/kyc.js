...
router.post("/sync", requireAuth, async (req, res) => {
  ...
  if (inquiryStatus.isApproved) {
    ...
    console.log(
      `[KYC] sync approved user ${userId} via direct Persona API check`,
    );
...
async function handleWebhookEvent(event) {
  ...
  if (!payload) {
    console.log(
      "[KYC webhook] no payload, raw event keys:",
      JSON.stringify(Object.keys(event?.data?.attributes ?? {})),
    );
    return;
  }
...
async function handleVerificationEvent(eventName, verification) {
  ...
  if (
    !verification.type?.startsWith("verification/government-id") &&
    !verification.type?.startsWith("verification/selfie")
  )
    return;
  ...
  if (!details?.referenceId || !details?.inquiryId) {
    console.warn(
      "[KYC] Could not resolve userId from verification",
      verification.id,
    );
    return;
  }
  ...
  console.log(
    `[KYC] Recorded verification ${verification.id} (${eventName}) — awaiting inquiry completion`,
  );
}
...
async function handleInquiryEvent(eventName, inquiry) {
  ...
  console.log(
    `[KYC inquiry] id=${inquiryId} userId=${userId} status=${personaStatus} attrs_keys=${Object.keys(attrs).join(",")} meta_keys=${Object.keys(meta).join(",")}`,
  );
  if (!userId) {
    // Dump a truncated snapshot so we can identify the correct field name
    console.log(
      "[KYC inquiry] FULL inquiry snapshot:",
      JSON.stringify(inquiry).slice(0, 800),
    );
  }
...
async function upsertKycResult({
  ...
  if (isApproved && details.legalName)
    userUpdate.legal_name = details.legalName;
    ...
    try {
      ...
      if (selfieUrl) {
        ...
        if (imgRes.ok) {
          ...
          console.log(
            `[KYC] Stored Persona selfie as profile photo for user ${userId}`,
          );
        }
      }
    } catch (err) {
      console.warn(
        `[KYC] Could not store selfie photo for user ${userId}:`,
        err.message,
      );
    }
  }
