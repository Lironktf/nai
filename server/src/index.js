...
app.get("/.well-known/apple-app-site-association", (_req, res) => {
  ...
  if (!teamId) {
    console.warn(
      "[AASA] APPLE_TEAM_ID env var not set — passkey domain verification will fail on real devices",
    );
  }
...
if (existsSync(clientDist)) {
  ...
  app.get("/liveness", (_req, res) =>
    res
      .status(503)
      .send("Liveness page not built. Run: cd client && npm run build"),
  );
}
...
if (existsSync(meetDist)) {
  ...
  app.get("/meet-panel", (_req, res) =>
    res.sendFile(join(meetDist, "index.html")),
  );
} else {
  app.get("/meet-panel", (_req, res) =>
